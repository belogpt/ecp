...
Если утилита не найдена, убедитесь, что CryptoPro установлен и путь к `cryptcp` добавлен в переменную окружения `PATH`. На Windows дополнительно проверяются типовые папки `C:\\Program Files\\Crypto Pro\\CSP\\cryptcp.exe`, `C:\\Program Files (x86)\\Crypto Pro\\CSP\\cryptcp.exe` и варианты с каталогом `CryptoPro`; на Linux — стандартные каталоги `/opt/cprocsp/bin/amd64/cryptcp` и `/opt/cprocsp/bin/ia32/cryptcp`.
